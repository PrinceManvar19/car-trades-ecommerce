...
if (isset($_POST['approve'])) {
    ...
    $stmt = $conn->prepare("UPDATE table_document_verification SET status = 'Approved' WHERE id = ?");
    $stmt->bind_param("i", $request_id);
    if ($stmt->execute()) {
        echo '<script>alert("Request approved successfully!")</script>';
        echo '<script>window.location.href = "Document Verification Requests.php";</script>';
    }
    $stmt->close();
}
...
if (isset($_POST['reject'])) {
    $request_id = filter_var($_POST['request_id'], FILTER_SANITIZE_NUMBER_INT);
    $stmt = $conn->prepare("UPDATE table_document_verification SET status = 'Rejected' WHERE id = ?");
    $stmt->bind_param("i", $request_id);
    if ($stmt->execute()) {
...
<head>
    ...
    <title>Document Verification Requests</title>
    <style>
        * {
    margin: 0;
    padding: 0;
...
<p>Document Verification ID: <?php echo $request['id']; ?></p>
    ...
    <img src="uploads/<?php echo $request['rc_book']; ?>" alt="RC Book Image">
</div>

<div class="image-container">
    <img src="uploads/<?php echo $request['Insurance']; ?>" alt="Insurance Image">
</div>
                    <form method="post">
