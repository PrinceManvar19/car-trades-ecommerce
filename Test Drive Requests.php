...
if (isset($_POST['approve'])) {
    ...
    $stmt = $conn->prepare("UPDATE test_drive SET status = 'Approved' WHERE Request_Id = ?");
    if ($stmt === false) {
    ...
    if ($stmt->execute()) {
        ...
        echo '<script>window.location.href = "Test Drive Requests.php";</script>';
    }
    $stmt->close();
}
...
if (isset($_POST['reject'])) {
    $request_id = filter_var($_POST['request_id'], FILTER_SANITIZE_NUMBER_INT);
    $stmt = $conn->prepare("UPDATE test_drive SET status = 'Rejected' WHERE Request_Id = ?");
    if ($stmt === false) {
        die("Error preparing statement: " . $conn->error);
    }
    $stmt->bind_param("i", $request_id);
        ...
        <ul class="request-list">
            <?php foreach ($test_drive_requests as $request) { ?>
                <li class="request-item">
                    <p>Test Drive ID: <?php echo $request['Request_Id']; ?></p>
